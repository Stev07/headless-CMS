...
if ( is_ssl() ) {
	...
	if ( ! defined( 'TECH_BANKER_BETA_URL' ) ) {
		define( 'TECH_BANKER_BETA_URL', 'http://captcha-bank.tech-banker.com' );
	}
}
...
if ( ! defined( 'CAPTCHA_BANK_VERSION_NUMBER' ) ) {
	define( 'CAPTCHA_BANK_VERSION_NUMBER', '4.0.28' );
}
...
if ( $version >= '4.0.4' ) {
		...
		function get_ip_location_captcha_bank( $ip_address ) {
			...
			if ( '' != $ip_location_meta_value ) {
				...
				if ( '' != $ip_location_array ) {
					...
					$wpdb->insert( captcha_bank_ip_locations(), $ip_location_array_data );
				}
	...
	if ( ! function_exists( 'blocking_visitors_captcha_bank' ) ) {
		...
		function blocking_visitors_captcha_bank() {
			...
			$flag         = 0;
			$country_code = '';
			$ip_address   = '::1' === get_ip_address_for_captcha_bank() ? sprintf( '%u', ip2long( '127.0.0.1' ) ) : sprintf( '%u', ip2long( get_ip_address_for_captcha_bank() ) );
